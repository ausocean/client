...
void TAS5805::play(const char* path) {
    ...
    BaseType_t ret = xTaskCreatePinnedToCore(
        [](void* arg) {
            ...
            if (f) {
                // --- THE SETVBUF TRICK ---
                // Create a 32KB staging buffer in internal DMA RAM
                ...
                p->instance->internal_play_loop(f);

                fclose(f);
                if (fs_buf) free(fs_buf);
            } else {
                ESP_LOGE("TAS5805", "Could not open file: %s", p->path);
            }
        ...
        20,   // High priority
        NULL,
        1     // Core 1 (Free from Wi-Fi/Ethernet interrupts usually)
    );
...
void TAS5805::internal_play_loop(FILE* f) {
    if (tx_handle == NULL || *tx_handle == NULL) return;

    wav_header_t header;
    if (fread(&header, sizeof(wav_header_t), 1, f) != 1) return;
    ...
    while ((samples_read = fread(file_buf, frame_size, samples_per_read, f)) > 0) {
        ...
        i2s_channel_write(*tx_handle, i2s_buf, 
                          samples_read * 2 * sizeof(int16_t), 
 ...
 */
int16_t calculate_sine(size_t sample_index, float target_freq, int sample_rate, int16_t amplitude) {
...
void TAS5805::play_beep(uint32_t duration_ms) {
    ...
    int16_t* dma_buffer = (int16_t*)heap_caps_malloc(buffer_size, MALLOC_CAP_DMA);
    ...
    if (dma_buffer) {
        ...
        while (samples_played < total_samples_to_play) {
            ...
            i2s_channel_write(*tx_handle, dma_buffer, buffer_size, &bytes_written, portMAX_DELAY);
...
void TAS5805::test_performance_gap() {
    ...
    int16_t* ram_buffer = (int16_t*)heap_caps_malloc(test_samples * 2 * sizeof(int16_t), MALLOC_CAP_DMA);
    ...
    for(int i=0; i<1000; i++) {
        ...
        i2s_channel_write(*tx_handle, ram_buffer, test_samples * 2 * sizeof(int16_t), &written, portMAX_DELAY);
    }
